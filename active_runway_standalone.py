...
class RunwayMonitor:
    ...
    def get_airport_info(self):
        ...
        if response.status_code != 200 or not response.text:
            raise Exception(f"Failed to get airport info: server returned HTTP {response.status_code} with empty response")
    ...
    def display_runway_info(self, weather_data, traffic_data):
        ...
        print("RWY    HDG(T) HDG(M)  Headwind XWnd(+L)    ARR (30m)    DEP (30m)")
        print("-" * 65)
    ...
    def run(self):
        ...
        try:
            ...
            raise  # Let main() handle deauth

def main():
    global print_callsign
    parser = ArgumentParser(description='Airport Runway Wind and Traffic Monitor')
    parser.add_argument('-a', '--airport', type=str, default='YSSY',
                       help='ICAO code of airport to monitor. Default: YSSY')
    ...
    parser.add_argument('-api', '--api', default="v6", help="API version to use")
    ...
    try:
        ...
        if monitor and hasattr(monitor, 'guid'):
            print("Deauthenticating...")
            requests.post(monitor.deauth_url, {"GUID": monitor.guid}, headers=monitor.header)
        sys.exit(0)
